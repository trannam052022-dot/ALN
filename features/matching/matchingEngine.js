...
(function(){
  ...
  function scoreKts(kts, meetProvince) {
    var prox   = proximityScore(kts.homeProvince, meetProvince);
    var rating = Math.min(5, Math.max(0, kts.rating || 3)) / 5 * 100;
    // weights: proximity 40%, rating 60% (style/budget deferred to Layer 2)
    return Math.round(prox * 0.4 + rating * 0.6);
  }

  function rankKts(ktsList, meetProvince, limit) {
    return ktsList
      .map(function(k){ return { kts: k, score: scoreKts(k, meetProvince) }; })
      .sort(function(a, b){ return b.score - a.score; })
      .slice(0, limit || 5);
  }

  window.ALNMatching = { rankKts: rankKts, PROV_LABEL: PROV_LABEL, NEARBY_MAP: NEARBY_MAP };
})();
