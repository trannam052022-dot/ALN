...
| `ncc-apply.html` | Form đăng ký Gian hàng NCC (nhà cung cấp vật liệu/thiết bị) → ghi `nccApplications/{uid}`, role `ncc`, username prefix `ncc.`. Đọc `?cat=` để pre-chọn danh mục (thêm 22/07/2026, dùng bởi `tho-thi-cong.html`) |
| `tho-thi-cong.html` | Trang công khai (thêm 22/07/2026) tuyển riêng "Thợ - Đội thi công" cho chiến dịch "Đầu quân về ALN" — hero + 3 giá trị (việc tìm đến đội, 15% C1, teaser sửa vặt) + thang tin cậy 3 bậc + dải đội thật (`nccApplications` category `dv_thicong`) + progress bar mục tiêu 20 đội. CTA → `ncc-apply.html?cat=dv_thicong` |
...
- **Bốc Chip May Mắn (thêm 23/07/2026, chốt bản 5 ô cùng ngày)** — trò chơi phản xạ riêng biệt, KHÔNG dùng chung điều kiện mở khoá với vòng quay tuần: mở khoá **vĩnh viễn** ngay sau khi hoàn thành ≥1 nhiệm vụ bất kỳ lần đầu (tức chỉ cần `users/{sđt}` tồn tại), sau đó chơi **1 lượt/ngày**. **5 ô chip** nhãn 1.000đ/2.000đ/3.000đ/4.000đ/5.000đ nhưng số Gạch thật là **5/10/15/20/25** (`CHIP_PRIZES` trong `functions/ctvGame.js`, PHẢI khớp `chip-cell` trong `ctv_dashboard.html`) — thấp hơn tỷ lệ quy đổi gốc 1 Gạch=100đ (lẽ ra là 10/20/30/40/50) vì tính lại thấy chi phí/ngày quá cao nếu chơi đều mỗi ngày. Trọng số xuất hiện GIẢM DẦN theo giá trị (45/25/15/10/5 — chip cao vừa hiếm vừa cửa sổ trúng ngắn 800→220ms) để độ khó tăng đúng như yêu cầu ban đầu. **(Bản đầu 4 ô 1.000/2.000/5.000/10.000đ, trọng số 55/30/12/3, đã thay hẳn — không còn dùng.)**
  - **Timing xác thực SERVER-SIDE** (khác hẳn honor-system của 4 nhiệm vụ cố định): `ctvChipStart` chốt sẵn ô + độ trễ + cửa sổ trúng trước khi trả về client, lưu tạm `ctvChipRounds/{sđt}`; `ctvChipClick` đối chiếu thời điểm gọi hàm thực tế (+ grace 150ms bù mạng) và ô client báo đã bấm với dữ liệu đã chốt — client không tự báo trúng được (khác spin wheel/nhiệm vụ vốn tin client hoàn toàn).
  - **Trần ngân sách TOÀN HỆ THỐNG/ngày** (không phải trần/người) — `CHIP_DAILY_BUDGET = 1000` Gạch/ngày (~100.000đ nợ ngầm tối đa nếu sau này Gạch quy đổi được ra tiền thật, xem tỷ lệ 1 Gạch=100đ), đếm qua `ctvChipBudget/{ngày}.spent`. Lý do phải thêm: chi phí/ngày của trò chơi **không có trần tự nhiên** — tăng tuyến tính theo số người chơi/ngày, mà đây lại đúng là mục tiêu tăng trưởng của game nên không thể chấp nhận rủi ro không giới hạn. `ctvChipStart` chặn mở lượt MỚI ngay khi đã chạm trần (không ai bị báo "trúng nhưng không nhận được" — vẫn minh bạch). Có thể vượt trần tối đa đúng bằng 1 giải cao nhất (25 Gạch) do cộng dồn ngay lúc trao thưởng thay vì giữ chỗ trước — chấp nhận được, đổi lại giữ đơn giản.
  - **Mùa Chip 90 ngày (Founder chốt 23/07/2026):** kể từ lần đầu 1 CTV chơi Chip (`ctvChipFirstPlayedAt` set 1 lần trong `ctvChipStart`), sau đúng 90 ngày **TOÀN BỘ `alnBricks`** của người đó (không riêng phần thắng từ Chip — mọi nguồn: nhiệm vụ, vòng quay, chip, mời bạn...) bị reset về 0, rồi mốc tự dời tới "bây giờ" nên mùa tiếp theo tự lặp lại vô thời hạn. Chu kỳ tính RIÊNG cho từng người (không phải lịch quý chung), thực thi bởi cron `ctvSeasonReset` (03:30 giờ VN hàng ngày, lọc `role=='ctv_lead'` trong bộ nhớ như `scanCtvSuspicion` để khỏi cần thêm index). **KHÔNG đụng** `alnDiamonds`/`referralPendingVnd` (tiền hoa hồng C1 thật đã kiếm — không được xoá) và **KHÔNG đụng** `ctvTasksDone` (nhiệm vụ cố định vẫn tính đã làm, tránh mở đường cày lại Gạch mỗi mùa). Có ghi ledger `ctv_season_reset_{ngày}_{sđt}` (amount âm = số Gạch bị xoá) để giữ đúng nguyên tắc "sổ cái bất biến", và push Founder tổng số CTV + tổng Gạch đã reset mỗi lần cron chạy có tác dụng. Đã hiển thị cảnh báo trước (không chỉ sau khi đã chơi) ngay trong mô tả trò chơi trên `ctv_dashboard.html`, cộng đếm ngược "còn X ngày" khi đã có mốc bắt đầu (`chip.daysUntilReset`).
...
- **Gói Hồ Sơ Bản Vẽ Kinh Tế — hoa hồng CTV (thêm 23/07/2026):** CTV có thẻ riêng trên `ctv_dashboard.html` giới thiệu **"Kho hồ sơ mẫu"** đã có sẵn trên `index.html` (mẫu nhà đã bàn giao, giá rẻ hơn thiết kế riêng) kèm link `index.html?ctvref={sđt}#khomau` — khách bấm "Đặt mua gói kinh tế" trên trang đó ghi 1 lead ẩn danh vào `mauNhaLeads` (qua `logMauNhaLead`, `functions/ctvGame.js`) kèm SĐT CTV nếu có, đồng thời push Founder ngay. **Hoa hồng 15%** (`MAU_NHA_COMMISSION_PCT`, cùng mức C1 nhưng biến riêng vì khác sản phẩm) — do gói kinh tế hiện CHỈ bán qua chat tư vấn (chưa có đơn hàng/thanh toán thật, xem hàm `datMua()` trong index.html), xác nhận "bán được" vẫn THỦ CÔNG: Founder vào `founder_panel.html` tab "Gạch & Kim Cương" → mục "Lead giới thiệu Gói Kinh Tế chờ xác nhận", bấm "Xác nhận đã bán" + nhập giá bán thật → `founderConfirmMauNhaSale` tự cộng 15% vào `users/{sđt}.referralPendingVnd` (CÙNG field dùng chung với hoa hồng C1 — Founder chi tay như nhau). Danh sách lead đọc qua `founderListMauNhaLeads` (onCall, Admin SDK) thay vì `getDocs` thẳng, vì **`mauNhaLeads` CHƯA mở rule đọc/ghi công khai trong `firestore.rules`** (tránh phải sửa rules) — mọi thao tác đều đi qua Cloud Functions.
...
- **Còn tồn đọng (chưa làm, chưa có xác nhận Founder):**
  ...
  6. **Chiến dịch "Đầu quân về ALN" — Thợ - Đội thi công (Founder CHỐT khung 22/07/2026):** danh mục `dv_thicong` là **lực lượng đầu quân** chuẩn bị cho dịch vụ sửa vặt. **Bắt buộc giữ khuôn pháp lý**: ALN CHỈ kết nối/chuyển lead, KHÔNG là bên cung cấp dịch vụ sửa chữa, KHÔNG thu tiền sửa chữa (cùng ranh giới "đơn vị thi công đối tác" — từng bị AI Overview hiểu sai 22/07/2026, xem mục JSON-LD). Kế hoạch 3 giai đoạn / 6 tuần:
     - **GĐ1 Tuyển quân (23/07→05/08):** bài Fanpage tuyển đội đầu tiên đã đăng qua MyMy Marketing 23/07 11:30 (tag `ncc-recruit-jul26`, trỏ `ncc-apply.html` — bài NÀY đã lên lịch trước khi có trang riêng, không sửa lại) + bản ngắn Founder tự đăng tay vào nhóm FB thợ xây + nhiệm vụ tuần CTV `share_group` lan truyền + Founder nhắn tay mời KTS giới thiệu đội quen (mạng lưới nhỏ, nhắn tay thân tình hơn — KHÔNG build cơ chế thông báo trong kts_dashboard) + bài Cẩm nang `cach-chon-doi-thi-cong-khi-da-co-ban-ve` tự đăng 24/07 (đẩy sớm từ 02/08 theo yêu cầu Founder). **Landing page riêng `tho-thi-cong.html` đã build 22/07/2026** — mọi bài tuyển ĐỘI THI CÔNG từ giờ trở đi (kể cả bài vinh danh GĐ2, quảng cáo trả phí nếu có) PHẢI trỏ về `tho-thi-cong.html` thay vì thẳng `ncc-apply.html` (trang riêng nói đúng ngôn ngữ đội thợ, tỷ lệ chuyển đổi cao hơn).
...
### Lịch nội dung Fanpage chuẩn theo tuần (chốt 22/07/2026)

Nguyên tắc nền (đã thống nhất với Founder): **luân phiên LOẠI nội dung, KHÔNG tăng số lượng bài** — Fanpage nhỏ đăng nhiều bài/ngày làm loãng tương tác từng bài, thuật toán giảm phân phối; sức mạnh thật là nhiệm vụ tuần CTV đảm bảo mỗi bài đều có bình luận thật. Tối đa 1 bài/ngày (tuần cao điểm chiến dịch được 2, cách nhau sáng/tối).

| Thứ | Loại bài | Giờ | Cách đăng |
|---|---|---|---|
| T2 | Cẩm nang kiến thức | 06:00 | Tự động (cron `publish-cam-nang.yml`) |
| T3 | Nghỉ / ảnh công trình, mẹo ngắn (khi có) | 11:30 | Đăng tay hoặc MyMy |
| T4 | Cẩm nang kiến thức | 06:00 | Tự động |
| T5 | Giới thiệu **nhiệm vụ tuần CTV mới** (task rotate sáng T5 ~07:00 VN theo tuần epoch) | 19:30 | MyMy, tag `content-weekly-*` |
| T6 | Cẩm nang kiến thức | 06:00 | Tự động |
| T7 | Bài tuyển (KTS/NCC/đội thi công — luân phiên theo chiến dịch đang chạy) | 11:30 | MyMy, tag `*-recruit-*` |
| CN | Vinh danh Top 3 CTV tuần (số liệu từ bảng xếp hạng trên `ctv_dashboard.html`) | 19:30 | Đăng tay hoặc MyMy |

Quy tắc kèm theo: (1) **mọi caption** đều gắn dòng nhiệm vụ CTV (vd *"🧱 CTV ALN: bình luận bài này = +20 Gạch + lượt quay may mắn → applamnha.vn/ctv_dashboard.html"*) để kích bình luận; (2) bài Cẩm nang MỚI viết đặt `publishDate` rơi vào T2/T4/T6 cho khớp lịch; (3) sau 1-2 tuần xem `getMarketingReport`, chỉ bơm ads nhỏ (200-500k) cho 1-2 bài tương tác tốt nhất — khuếch đại bài đã chứng minh hiệu quả, không rải đều.
