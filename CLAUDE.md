...
- Backend: `functions/ctvGame.js` — 2 Cloud Function `onCall` KHÔNG yêu cầu `request.auth` (App Check vẫn áp dụng như mọi onCall khác): `ctvGetProfile({phone})` tra Gạch/Kim Cương, `ctvClaimTasks({name,phone,taskIds})` cộng Gạch idempotent (ledger `ctv_task_{taskId}_{sđt}`, chặn nộp trùng). Ghi vào `users/{sđt}` — **doc ID là số điện thoại**, không có Auth account tương ứng, `role:'ctv_lead'`. Đi qua Admin SDK nên không cần mở `firestore.rules` cho đọc/ghi ẩn danh.
- Link giới thiệu sau khi nhận thưởng lần đầu: `register.html?cref={sđt}` → ghi field `referredByCtv` (giá trị là SĐT, khác định dạng uid Firebase của NCC) lên user CN mới đăng ký. `bricksOnFirstPayment` (`functions/bricks.js`) đã đọc field này song song `referredByNcc` — CTV giới thiệu CN ký HĐ + đóng C1 thật sẽ tự động nhận Kim Cương vào đúng `users/{sđt}`.
- Nhiệm vụ hiện có 4 loại (sửa ở `CTV_TASKS` trong `functions/ctvGame.js` + mảng `TASKS` trong `ctv_dashboard.html`, phải sửa ĐỒNG BỘ cả 2 nơi): theo dõi Fanpage, quan tâm Zalo OA, bình luận bài viết, chia sẻ link. **Chưa có** nhiệm vụ "đi chợ giùm"/"nhật ký xây nhà" (cần nộp ảnh/bài, phức tạp hơn honor-system đơn giản hiện tại).
...
- **CHƯA deploy** — cần chạy `firebase deploy --only "functions:ctvGetProfile,functions:ctvClaimTasks,functions:bricksOnFirstPayment"`.
...
7. ~~Không có cửa vào cho người ngoài mạng lưới ALN~~ — đã làm 22/07/2026, **2 lần**: lần 1 làm vai `ctv` có tài khoản (Firebase Auth, `register.html` tab riêng) — Founder thấy vẫn rườm rà, yêu cầu đơn giản hơn nữa. Lần 2 (cùng ngày) **thay thế hoàn toàn**: `ctv_dashboard.html` giờ là trang công khai không cần đăng nhập, `functions/ctvGame.js` xử lý qua SĐT — xem chi tiết mục "Game Cộng tác viên công khai" phía trên (ngay sau bảng Vai trò hệ thống). Kim Cương tự động khi CTV giới thiệu CN ký HĐ + đóng C1 **đã nối xong** (`bricksOnFirstPayment` đọc `referredByCtv`). Hình ngôi nhà xây dần theo Gạch (`alnHouseSVG`) có ở cả `ctv_dashboard.html` lẫn `client_CN/kts_dashboard/designer_dashboard/ncc-dashboard`, **chưa có ở `founder_panel.html`** (bảng xếp hạng dạng bảng, chưa cần hình — lưu ý bảng xếp hạng đó sẽ KHÔNG thấy người chơi CTV ẩn danh vì họ không phải Firebase Auth user, cần query thêm nếu muốn Founder xem cả nhóm này). **Còn thiếu:** nhiệm vụ "đi chợ giùm"/"nhật ký xây nhà" (cần nộp ảnh/bài, chưa hợp với honor-system đơn giản hiện tại); vòng quay may mắn (yếu tố ngẫu nhiên) và bảng xếp hạng công khai cho người chơi tự xem — mới là ý tưởng, chưa làm; **CHƯA deploy** 2 hàm mới + bản sửa `bricksOnFirstPayment`.
