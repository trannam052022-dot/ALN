...
- Link giới thiệu sau khi nhận thưởng lần đầu: `register.html?cref={sđt}` → ghi field `referredByCtv` (giá trị là SĐT, khác định dạng uid Firebase của NCC) lên user CN mới đăng ký. `bricksOnFirstPayment` (`functions/bricks.js`) đọc field này song song `referredByNcc` — từ 22/07/2026 (chốt chiến lược cùng ngày) 2 đường TÁCH BIỆT: NCC giới thiệu vẫn nhận **1 Kim Cương** như cũ; **CTV giới thiệu nhận thẳng 15% tiền C1 thật bằng VNĐ** (`CTV_REFERRAL_PAYOUT_PCT`, cộng dồn `users/{sđt}.ctvReferralPendingVnd`, ledger `ctv_payout_c1paid_{pid}` unit `vnd`, push Founder "💰 Thưởng giới thiệu CTV"). **Chưa có luồng RÚT tiền tự động** (OTP Firebase Phone Auth + form ngân hàng + khấu trừ thuế TNCN) — số dư chỉ tích luỹ hiển thị, Founder chi tay khi nhận push.
