...
- **Rules:** `firestore.rules` đã thay hẳn block `_draft` bằng block production (collection thật, cùng cấu trúc quyền, `forumPosts` thay vì `alnPosts`). ✅ Đã deploy (05/07/2026, PR #7 + `firebase deploy --only firestore:rules`).
...
- [ ] **Dọn 13 collection `*_draft` cũ** trên Firestore Console (đã migrate xong, an toàn để xoá): `alnPosts_draft`, `reports_draft`, `modLogs_draft`, `modQueue_draft`, `invites_draft`, `leads_draft`, `forumConfig_draft`, `ktsReputation_draft`, `forumUserState_draft`, `notifBuffer_draft`, `camNangForum_draft`, `forumDigest_draft`, `hoiKtsQueue_draft`.
- [ ] **Theo dõi chi phí** Cloud Vision (OCR) + Claude API vài ngày đầu có traffic thật — xem hướng dẫn nơi kiểm tra ở lịch sử chat phiên 05/07/2026. Cân nhắc đặt Budget Alert trên Google Cloud Billing cho `aln-platform`.
