...
- **D3 (badge "KTS Sáng lập") — ĐÃ GỠ BỎ theo yêu cầu Founder (07/07/2026):** ban đầu có
  làm (đọc `kts_profiles_draft.badgeFounder`, hiện badge cạnh tên KTS trên bài đăng/bình
  luận), nhưng Founder yêu cầu KHÔNG hiển thị bất kỳ huy hiệu/chữ "Sáng lập" nào trên các
  trang — ai đứng đầu (kể cả chính Founder với vai KTS) chỉ nên nổi bật tự nhiên qua bảng
  xếp hạng điểm uy tín (D4) như mọi KTS khác, không cần nhãn riêng. Đã gỡ hoàn toàn khỏi
  `forum.html` (function `founderBadge`/`loadFounderBadges`, biến `KTSC_FOUNDER_UIDS`, 3 chỗ
  gọi trên feed/thread/comment) và khỏi `kts_landing_draft.html` (khối `.pf-badge` "KTS SÁNG
  LẬP"). **Vẫn giữ field `badgeFounder`** trong `kts_profiles_draft` + checkbox quản lý trong
  `founder_kts_creator_draft.html` (chỉ Founder thấy, dùng nội bộ để đánh dấu/theo dõi nhóm
  KTS pilot đầu tiên — không hiển thị công khai).
