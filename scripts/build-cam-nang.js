...
//   4. Gọi tools/gen-sitemap.js sinh lại sitemap.xml — DUY NHẤT MỘT nguồn
//      whitelist (SEO_BAN_GIAO.md Phase 0), KHÔNG tự sinh sitemap riêng ở
//      đây nữa (trước 11/07/2026 có buildSitemap() riêng, không biết tới
//      mau/, du-toan/, và không theo whitelist → mỗi lần bot này chạy lại
//      xoá mất các trang đó khỏi sitemap + đưa lại các trang bị loại như
//      register.html/*-apply.html — xem SEO_BAN_GIAO.md).
//   (KHÔNG tự sinh robots.txt — file này giờ maintain tay, xem robots.txt)
//
...
// Gọi tools/gen-sitemap.js làm subprocess — DUY NHẤT một nguồn whitelist cho
// toàn site (xem comment đầu file). Script đó tự quét cam-nang/ + mau/ +
// du-toan/ + STATIC_PAGES riêng của nó, không cần truyền "articles" vào.
function buildSitemap() {
  var before = fs.existsSync(path.join(ROOT, 'sitemap.xml'))
    ? fs.readFileSync(path.join(ROOT, 'sitemap.xml'), 'utf8') : null;
  execSync('node tools/gen-sitemap.js', { cwd: ROOT, stdio: 'inherit' });
  var after = fs.readFileSync(path.join(ROOT, 'sitemap.xml'), 'utf8');
  return before !== after;
}

// ── Section "Cẩm nang làm nhà" trên index.html (root — GitHub Pages trả về
// tại '/') — CHỈ ghi đè phần nằm giữa 2 marker dưới đây (danh sách thẻ 3 bài
// mới nhất). Không đụng gì khác. Trước 11/07/2026 nhắm vào home.html, nhưng
// từ SEO Phase 0 (đổi vai trò index.html <-> home.html để canonical đúng
// domain gốc) nội dung trang chủ thật đã chuyển sang index.html — home.html
// giờ chỉ là trang redirect, không còn marker. Nếu marker không tồn tại, bỏ
// qua bước này thay vì lỗi.
...
async function main() {
  ...
  var sitemapChanged = buildSitemap();
  ...
  console.log('  index.html (khối 3 bài mới nhất) đổi: ' + (homeChanged ? 'có' : 'không'));
